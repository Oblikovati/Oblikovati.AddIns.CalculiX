...
| 7 | **No node-conformal mesh across body interfaces** — each body is meshed independently, so touching bodies don't share interface nodes | **resolved for coincident planar interfaces:** auto-detected `*TIE` bonds the non-conformal meshes ([ADR-0005](ADR-0005-tie-constraints-bonded-bodies.md)); disjoint bodies still solve independently | extend tie detection to partial-overlap / curved interfaces, and conformal combined-geometry meshing |
