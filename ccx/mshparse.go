...
const (
	gmshTet4  = 4  // 4-node linear tetrahedron  -> C3D4
	gmshTet10 = 11 // 10-node quadratic tetrahedron -> C3D10
...
// re-ordered from gmsh's node numbering to CalculiX's C3D10 numbering; boundary
...
func addElement(mesh *TetMesh, nums []int) {
	...
	switch etype {
	case gmshTet10:
		mesh.Elements = append(mesh.Elements, TetElement{ID: id, Nodes: gmshTet10ToCcx(nodes)})
	case gmshTet4:
...
// gmshTet10ToCcx reorders a gmsh 10-node tetrahedron into CalculiX C3D10 numbering.
// The corner nodes (1-4) and the first four mid-edge nodes match; gmsh's last two
// mid-edge nodes are swapped relative to Abaqus/CalculiX (gmsh node 9 = mid(3,4),
// node 10 = mid(2,4); CalculiX wants them the other way round). Same permutation the
// gmsh2ccx converter applies.
func gmshTet10ToCcx(g []int) []int {
	if len(g) != 10 {
