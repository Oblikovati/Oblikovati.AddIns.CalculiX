...
// solidBodies returns the active part's solid bodies — the ones a stress/thermal study
// meshes. Non-solid bodies (surfaces, wires) are skipped.
...
// conformal (a documented limitation; bonded contact would need tie constraints).
func (e *Engine) meshSolidBodies(bins solverBinaries, settings StudySettings, solids []wire.BodyInfo, dir string) (*TetMesh, error) {
	...
	for i, b := range solids {
		...
		m, err := NewGmshMesher(bins.gmsh).Mesh(surface, opts, bodyDir)
		if err != nil {
