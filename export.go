...
// Command oblikovati-calculix is built as a c-shared library (.so/.dll/.dylib) and
...
// `make sync-header`): on Activate it constructs the CalculiX FEA engine (package ccx)
// bound to the host through the host-supplied callback. The host owns the model; this
// library owns the surface→volume-mesh→deck→solve→render pipeline.
package main
...
import (
	...
	"oblikovati.org/api"
	"oblikovati.org/calculix/ccx"
)

const addInID = "com.oblikovati.calculix"
...
var (
	...
	engine   *ccx.Engine   // active FEA engine, nil when inactive
)
...
func ObkAddInActivate(call C.ObkHostCall, freeFn C.ObkHostFree) C.int {
	...
	eng := ccx.NewEngine(cgoHostCaller{})
	engine = eng
	...
	// the dispatcher — so registering inline would deadlock the head (same pattern as the FEMM
	// bridge + MCP bridge). The running frame loop drains this goroutine's host calls.
	go func() { _ = eng.Setup() }()
	return C.OBK_OK
}
